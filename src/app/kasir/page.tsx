...
function buildDailyReport(orders: Order[], date: string): string {
  ...
  const lines = [
    ...
    ``,
    `_Laporan otomatis dari Basecamp POS_ 🚀`,
...
function showBrowserNotif(title: string, body: string) {
  ...
  if (Notification.permission === 'granted') {
    new Notification(title, { body, icon: '/favicon.svg', tag: 'basecamp-order' })
  }
}
...
function msgSiap(order: Order) {
  ...
  return `Halo ${order.customer_name || 'Kak'}! 👋\n\nPesananmu di *Basecamp* sudah siap diambil. 🔔\n\n🪑 ${meja}\n💳 Pembayaran: ${bayar}\n\nSilakan ke kasir ya! ☕`
}
...
export default function KasirPage() {
  ...
  useEffect(() => { if (localStorage.getItem('basecamp-kasir') === 'ok') setAuthed(true) }, [])
  ...
  useEffect(() => {
    ...
    document.title = count > 0 ? `(${count}) Order Baru | Basecamp Kasir` : 'Basecamp Kasir'
  }, [newOrders])
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      localStorage.setItem('basecamp-kasir', 'ok')
      setAuthed(true)
    ...
    <div className="min-h-screen bg-h-bg flex items-center justify-center p-6">
      <div className="bg-h-card border border-h-border rounded-2xl p-8 w-full max-w-sm">
        <div className="text-center mb-8">
          <div className="font-sans text-2xl font-black text-white tracking-widest uppercase">BASECAMP</div>
    ...
    <div className="min-h-screen bg-h-bg" onClick={resetIdle}>
      ...
      <header className="bg-h-dark border-b border-h-border">
        <div className="max-w-4xl mx-auto px-5 py-4 flex items-center justify-between">
          <div>
            <div className="font-sans text-xl font-black text-white tracking-widest uppercase">BASECAMP</div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => { localStorage.removeItem('basecamp-kasir'); setAuthed(false) }}
