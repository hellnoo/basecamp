...
function exportCsv(orders: OrderRow[]) {
  ...
  a.download = `basecamp-rekap-${new Date().toISOString().slice(0, 10)}.csv`
...
export default function AdminPage() {
  ...
  useEffect(() => { if (localStorage.getItem('basecamp-admin') === 'ok') setAuthed(true) }, [])
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (res.ok) { localStorage.setItem('basecamp-admin', 'ok'); setAuthed(true) }
      ...
      <header className="bg-h-dark border-b border-h-border">
        <div className="max-w-5xl mx-auto px-5 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => { localStorage.removeItem('basecamp-admin'); setAuthed(false) }}
