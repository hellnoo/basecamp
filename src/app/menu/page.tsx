...
function generatePlaceholder(item: MenuItem): string {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="400" height="200" viewBox="0 0 400 200">
    ...
    <text x="200" y="178" text-anchor="middle" font-family="system-ui,sans-serif" font-size="10" fill="rgba(212,184,150,0.45)" letter-spacing="5">BASECAMP</text>
  </svg>`
...
function ChatbotWidget({ items }: { items: MenuItem[] }) {
  ...
  const [messages, setMessages] = useState<{ role: 'user' | 'assistant'; content: string }[]>([
    { role: 'assistant', content: 'Halo Kak! ☕ Aku barista AI BASECAMP. Mau rekomendasi menu? Tanya aja, atau pilih cepat di bawah 👇' }
  ])
...
const ORDER_KEY = (table: string) => `basecamp-order-${table}`
...
function MenuContent() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem(`basecamp-cart-${tableNum}`)
    if (saved) try { setCart(JSON.parse(saved)) } catch { /* ignore */ }
  }, [tableNum])
  useEffect(() => {
    if (Object.keys(cart).length > 0) localStorage.setItem(`basecamp-cart-${tableNum}`, JSON.stringify(cart))
    else localStorage.removeItem(`basecamp-cart-${tableNum}`)
  }, [cart, tableNum])
  ...
  if (restoring) return (
    <div className="min-h-screen bg-h-bg flex items-center justify-center">
      <div className="text-center">
        <div className="font-sans font-black text-white tracking-widest text-xl uppercase mb-2">BASECAMP</div>
      ...
      <div className="min-h-screen bg-h-bg flex flex-col items-center justify-center text-center px-8">
        {isCancelled ? (
          ...
          <>
            ...
            <p className="text-h-muted text-xs mt-3 max-w-xs">Terima kasih sudah mampir ke BASECAMP ☕</p>
            {/* Rating */}
            {!rated ? (
        ...
        <div className="mt-8 flex flex-col items-center gap-3">
          ...
          {(isCancelled || isDone) && (
            <button
              onClick={() => { setCart({}); setNote(''); setCustomerName(''); setPhone(''); setPayMethod(''); setSubmitted(false); setOrderId(null); clearActiveOrder(tableNum); localStorage.removeItem(`basecamp-cart-${tableNum}`) }}
    ...
    <div className="min-h-screen bg-h-bg">
      <header className={`bg-h-dark border-b border-h-border sticky z-40 ${orderId && !submitted ? 'top-[42px]' : 'top-0'}`}>
        <div className="max-w-[480px] mx-auto px-5 py-3.5 flex items-center justify-between">
          <div>
            <div className="font-sans text-lg font-black text-white tracking-widest uppercase leading-none">BASECAMP</div>
